    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, 6))
        {
            ...
            if (hit.transform.gameObject.CompareTag("Player"))
            {
                ...
                //se o contador for < 0 chama a funçao no objeto ButtonAction()
                if (counter < 0)
                {
            ...
            else if(hit.transform.gameObject.CompareTag("JumpScare"))
            {
                SceneManager.LoadScene("JumpScare"); //Transita para a cena de jumpscare
			}

            else if(hit.transform.gameObject.CompareTag("PlayGame"))
            {
                counter -= Time.deltaTime;
                if (counter < 0);
                {
                    SceneManager.LoadScene("level1");  //ativa o botao de play game no menu e transita para a cena level1
                    counter = 2; //resetar counter denovo
                }
			}

            else if(hit.transform.gameObject.CompareTag("Quit"))
            {
                Application.Quit();
			}
